...
| REQ-004 | A new module `app/energy_provider.py` defines the abstract `EnergyProvider`, the Phase-1 `StakingEnergyProvider` implementation (lifted verbatim from current `delegate_energy` inner function), and the `get_energy_provider()` factory. The Phase-2 `RefeeEnergyProvider` is covered by REQ-002/REQ-007/REQ-008/REQ-009. The `acquire` contract is energy-unit based: `acquire(receiver, energy_to_provision, account_resource, *, minimum_energy_required=None) -> bool`; the keyword exists so staking top-ups can delegate only `energy_diff` while still verifying the full transfer-wide `energy_needed`, matching the existing closure behavior. | spike 002 recommendation + Phase 1 plan review |
| REQ-005 | `transfer_trc20_from` in `app/tasks.py` is modified to use `provider.acquire(receiver, energy_to_provision, account_resource, minimum_energy_required=energy_needed)` at the existing delegation point (currently L354), and `provider.release(receiver)` at the existing undelegate point (currently L412-416). | spike 002 |
